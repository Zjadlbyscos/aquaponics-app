...
const ModuleDetail = () => {
  ...
  return (
    <div>
      ...
      <p style={{ color: currentTemperature >= (module.targetTemperature - 0.5) && currentTemperature <= (module.targetTemperature + 0.5) ? 'green' : 'red' }}>
        Current Temperature: {currentTemperature}°C
      </p>
      <button onClick={() => setShowEditDialog(true)} disabled={!module.available}>Edit Module</button>
      <Link to="/">Back to List</Link>

      {showEditDialog && <EditModuleDialog module={module} onClose={() => setShowEditDialog(false)} />}
